...
```
```{r}
pred_NB_corazon <- predict(NBclassfier, newdata = data_norm_corazon.test_set, type = "class")
...
## Predicción corazón
```{r}
nn_corazon <- neuralnet("In_hospital_death ~ HR_mean + NIDiasABP_mean + NIMAP_mean + NISysABP_mean + PaCO2_mean + PaO2_mean + pH_mean", 
...
#cat(sprintf("Best error reached: %f", nn_corazon$result.matrix[c('error'), ]))
```

```{r}
metrics(nn_corazon, data_tidy_corazon.test_set)
```
## Predicción sangre
```{r}
set.seed(101)
nn_corazon_norm <- neuralnet("In_hospital_death ~ HR_mean + NIDiasABP_mean + NIMAP_mean + NISysABP_mean + PaCO2_mean + PaO2_mean + pH_mean", 
                         data = data_norm_corazon.train_set,
                         hidden = c(4,8,8,4),
                         stepmax=1e6,
                         algorithm = "backprop",
...
plot(nn_corazon_norm)
```

```{r}
pred <- predict(nn_corazon_norm, data_norm_corazon.test_set)
#data.frame(data_norm_sangre.test_set$In_hospital_death,round(pred))
metrics(nn_corazon_norm, data_norm_corazon.test_set)
```
