...
Con los datos que disponemos, se puede predecir "la muerte en hospital" (in hospital death) de muchas formas. Pero en nuestro caso se ha decidio hacer algo un tanto distinto. Se intentará averiguar si hay algún órgano que sea el causante "directo" de muerte.

Para ello, se clasificarán las variables en grupos según al órgano al que pertenezcan. Una vez se tengan los grupos, se escogerá un modelo aducado y se predecirá la variable "muerte en hospital" con cada grupo de variables. El grupo con mayor porcentaje de aciertos será el órgano que tenga más probabilidades de ser un órgano crítico.


#  Ingesta de datos

Después de hacer un ejercicio de investigación, se han detectado los siguientes grupos de variables:

*Lista de grupos de variables*

De estas variables no se van a utilizar muchas de ellas debido a la ausencia de estas en muchos pacientes. Para determinar el porcentaje de valores NAs de la cada variable se ha utilizado un pequeño algoritmo.
...
Para recoger los datos, se utilizará el código que nos facilitó el profesor Ricardo.
