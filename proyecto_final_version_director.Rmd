...
Hay un error en al lectura pues toma minutos y segundos en lugar de horas y minutos. Para poder tener libertad  para tratar Time `

```{r}
series_parameters2 = series_parameters %>% mutate(Time=as.character(Time)) %>% separate(Time,into=c("sobra","horas","minutos"),sep=":") %>%
  select(-sobra) %>% mutate(TimeM=as.numeric(horas)*60+as.numeric(minutos)) %>% select(TimeM,Parameter,Value,RecordID)
```
